        ...
        let err = match try_get_displays() {
            Ok(mut displays) => {
                if displays.len() <= display_idx {
                    ...
                    match create_capturer(privacy_mode_id, display, true, display_idx) {
...
fn get_capturer(
    ...
    //     }
    // }
    let capturer = create_capturer(0, display, use_yuv, current)?;
    Ok(CapturerInfo {
        name,
        origin,
        width,
...
fn run(vs: VideoService) -> ResultType<()> {
    ...
    let sp = vs.sp;
    let mut c = get_capturer(display_idx, true)?;
...
fn get_recorder(
    ...
    let recorder = if !Config::get_option("allow-auto-record-incoming").is_empty() {
        ...
        Recorder::new(RecorderContext {
            server: true,
            id: Config::get_id(),
            default_dir: "".to_string(), //crate::ui_interface::default_video_save_directory(),
            filename: "".to_owned(),
            width,
            height,
